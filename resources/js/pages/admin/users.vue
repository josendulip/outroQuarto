    ...
    <sui-table single-line class="text-center">
      <sui-table-header>
        <sui-table-row>
          ...
          <sui-table-header-cell>
            <router-link class="ui button basic fluid tiny secondary" :to="{ name: 'admin.new.user' }">
              {{ $t('myadmin_user_create') }}
            </router-link>
          </sui-table-header-cell>
        </sui-table-row>
      </sui-table-header>
      <sui-table-body>
        <sui-table-row v-for="user in users.data" :key="user.id">
          <sui-table-cell>
            <a v-if="user.status == 'Active'" class="ui green empty circular label" />
            <a v-if="user.status == 'Inactive'" class="ui red empty circular label" />
          </sui-table-cell>
      ...
      Swal.fire({
        ...
        if (result.value) { // se o resultado for verdadeiro
          ...
          this.form.delete('/api/my-users/' + id).then(() => {
            // eslint-disable-next-line no-undef
            Swal.fire(this.$t('deleted'), this.$t('msg_deleted_success'), 'success')
            // eslint-disable-next-line no-undef
