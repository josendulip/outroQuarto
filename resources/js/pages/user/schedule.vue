<template>
  ...
  <div class="row justify-content-md-center">
    ...
    <div class="col-7" v-show="message">
        <span v-if="schedule.status === 'confirmed' || schedule.status === 'resolved'">
            <p>Caro(a) Usuário (a), </p>
            <p>O(A) Senhor(a) recebeu a confirmação de  visita  do imóvel com ID: <a href="#" @click.prevent="viewHouse(schedule.house_code)">{{schedule.house_code}}</a> para <strong class="font-weight-bold">
            {{schedule.date | dddd}},
            {{schedule.date | LL}} 
            às {{schedule.time}}
            </strong> 
            Proprietário <a href="javascript:void(0)" @click.prevent="viewOwner(ownerProperty.slug)">{{ownerProperty.name}}</a>, <a href="javascript:void(0)" @click.prevent="viewOwner(ownerProperty.slug)">{{ownerProperty.phone}}</a>, <a href="javascript:void(0)" @click.prevent="viewOwner(ownerProperty.slug)">{{ownerProperty.email}}</a>.</p>
            <p>Por favor contacte o proprietário para informar-se do local de encontro</p>
            <p>OutroQuarto agradece pela preferencia</p>  
        </span> 
        <span v-else>
            <p>Caro(a) Usuário (a), </p>
            <p>O(A) Senhor(a) solicitou visita  do imóvel com ID: <a href="#" @click.prevent="viewHouse(schedule.house_code)">{{schedule.house_code}}</a> para <strong class="font-weight-bold">
            {{schedule.date | dddd}},
            {{schedule.date | LL}} 
            às {{schedule.time}}
            </strong> 
            Proprietário <a href="javascript:void(0)" @click.prevent="viewOwner(ownerProperty.slug)">{{ownerProperty.name}}</a>.</p>
            <p>Por favor aguarde a confirmação.</p>
            <p>OutroQuarto agradece pela preferencia</p>  
        </span>     
        <hr>
        ...
        <div class="col-9 px-0">
            <div class="d-flex justify-content-between" v-if="schedule.status === 'resolved'">
                ...
                <router-link class="ui basic button" :to="{ name: 'user.schedules' }" basic> <sui-icon name="x icon" color="grey"/>{{$t('schedulStatus_cancel_btn')}}</router-link>
            </div>
            <div class="d-flex justify-content-between" v-else>
                <sui-button size="mini" basic v-if="schedule.status === 'confirmed'"><sui-icon name="check" color="green"/> {{ $t('schedulStatus_confirm') }}</sui-button>
                ...
                <sui-button size="mini" basic @click="rescheduleShow()"> <sui-icon name="undo alternate" color="blue"/>{{ $t('schedulStatus_schedul_btn') }}</sui-button>
                <sui-button size="small" basic @click="deleteSchedule(schedule.id)"><sui-icon name="trash alternate outline" color="red"/> {{ $t('property_delete') }}</sui-button>
                <router-link class="ui basic button" :to="{ name: 'user.schedules' }" basic> <sui-icon name="x icon" color="grey" /> {{$t('schedulStatus_cancel_btn')}}</router-link>
            </div>
        </div>
    </div>
...
export default {
  ...
  data(){
    ...
    methods:{
        viewOwner(slug) {
            this.$router.push("/view/owner-profile/" + slug);
        },
        ...
        try {
            ...
            this.$toastr.s(
                this.$t('schedule_msg_reschedul_success_user') 
            );
    ...
    ownerInfo(){
        axios.get("/api/owner-property/" + this.$route.params.id)
        .then(({data}) => {       
            this.ownerProperty = data;
        })
        .catch((error) => {
            this.$toastr.defaultStyle = { "background-color": "#FF5859" };
            this.$toastr.s(this.$t('schedule_message_error'));
    ...
    deleteSchedule(id){
            Swal.fire({
                ...
                confirmButtonText: this.$t("button_delete")
                    ...
                    this.form.post('/api/my-schedule/'+id).then(()=>{
...
                                'success'
                        ...
                        Fire.$emit('AfterCreated'); 
                        this.$router.push({'name': 'user.schedules'});  
                    }).catch(()=>{
