  ...
  <div class="">
          ...
          <sui-card class="content-house rounded-radius">
            ...
            <div class="button-house">
              <div is="sui-button-group">
                ...
                <sui-button @click.prevent="viewHouse(house.house_code)" icon="eye icon"/>
                <!-- {{ $t("myPanel_card_footer_view") }} -->
                <sui-button  @click="deleteHouse(house.id)" icon="minus circle"/>
              </div>
                <button class="btn btn-warning mb-0 button-down text-white" @click="addImages(house.house_code)">{{ $t('property_add_img') }}</button>
            </div>
            <sui-card-content>
              <sui-card-header
                >{{
        ...
        <sui-table basic="very">
          <sui-table-header>
            <sui-table-row>
              ...
              <sui-table-header-cell class="text-capitalize">{{ $t("annou_form_state") }}</sui-table-header-cell>
              ...
              <sui-table-header-cell class="text-capitalize">{{ $t("view_house_details_city")}}</sui-table-header-cell>
              <sui-table-header-cell class="text-capitalize">{{
                $t("mypanel_action")
              }}</sui-table-header-cell>
            </sui-table-row>
    ...
    <transition name="fade" mode="out-in">
      <div class="row column" v-if="cube">
        <div class="col-md-3 mb-4 px-1" v-for="house in myHouses.data" :key="house.id">
          <div class="Mycard">
    ...
    <!-- NONE -->
    <div class="row justify-content-center" v-if="myHouses.data <= 0">
      <div class="ui small message">
        <h3 class="main-color">
          <i class="exclamation triangle icon"></i> {{ $t("mypanel_message") }}
        </h3>
        <p class="text-blue d-block text-center">
          <router-link :to="{ name: 'my-panel.new.home' }"
            ><i class="bullhorn icon"></i> {{ $t("owner_house") }}</router-link>
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    addImages(house_code){
      this.$router.push("/add-images/" + house_code);
      },
