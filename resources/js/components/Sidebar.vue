      ...
      <template v-if="links.length">
            <div class="container">
                <div class="d-flex justify-content-lg-start align-items-center avatar-div">
                  <router-link  :to="{ name: 'my-panel.profile' }" class="p-0 m-0">
                ...
                <hr>
            <div class="container">
                <h3 class="text-white"><router-link :to="{  name: 'my-panel.welcome' }">{{$t('ownerpanel')}}</router-link></h3>
...
export default {
  ...
  computed: {
    ...
    app () {
      ...
      return document.getElementById('my-panel') 
    },
    navBar () {
      //return document.getElementById('app') //era esse que estava la
  ...
  mounted () {
    if (!this.app) {
      console.warn(`[Sidebar] The format was set to '${this.format}', but there is no element with an id 'app'. Add id='my-panel' to the element to move.`)
    }
  },
  methods: {
    openMenu () {
